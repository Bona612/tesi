    ...
    IERC6956
{
    ...
    // VEDERE SE FARE COSì
    mapping(uint256 => bytes32) public ipfsHashByToken;
        ...
        authorized(Role.ASSET, createAuthorizationMap(approveAuthorization))
    {
        ...
        _approve(to, tokenByAnchor[anchor], address(0));
    }
    ...
    //     address previousOwner = ERC6956._update(to, tokenId, _msgSender());
    ...
    function _safeMint(address to, bytes32 anchor, string memory cid) internal virtual {
    ...
    function createAnchor(bytes memory attestation, string memory cid, bytes memory data) public virtual
    {      
        bytes32 anchor;
        address to;
        bytes32 attestationHash;
    ...
    function createAnchor(bytes memory attestation, string memory cid) public virtual {
        return createAnchor(attestation, cid, "");
    }
