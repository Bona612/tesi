 ...
 *      With createAttestation the ERC-165 identifier is 0xf3cdf0f3
 *      Maybe is changed again because I changed AnchorTransfer event with the metadataURI
 */
interface IERC6956 {
     ...
     * @param tokenId ID (>0) of the anchored token
     */
    event AnchorTransfer(address indexed from, address indexed to, bytes32 indexed anchor, uint256 tokenId, string cid);
    /**
    ...
    function createAnchor(bytes memory attestation, string memory cid) external;
    function createAnchor(bytes memory attestation, string memory cid, bytes memory data) external;
